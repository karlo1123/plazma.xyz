		...
		local cfglocation = "plazma.xyzcfg/" 
		makefolder("plazma.xyzcfg") 
		...
		function library:New(name) 
			...
			TextLabel.Text = "plazma.xyz | Build : 1.0 | Private"
			...
			Menu.Size = UDim2.new(0, 621, 0, 598)
...
UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
	...
	return menu 
		...
		makefolder("plazmalua") 

		local allluas = {} 

		for _,lua in pairs(listfiles("plazmalua")) do 
			local luaname = GSUB(lua, "plazmalua\\", "") 
		...
		luascripts:Element("Button", "load", {}, function() 
			loadstring(readfile("plazmalua\\"..values.luas["lua scripts"].lua.Scroll))() 
		end) 
...
setreadonly(mt, true)
			...
			if tbl.Toggle then
				...
				if thing == true then
					...
					local chatmessages = {
						...
						"Seems like u died",
						"Did You die? aww",
						"DIED to NIXUS.WIN? ew imagine",
						"1",
						"NIXUS.WIN whitelist?, right none",
						"You died to a OLD NON-UPDATING CHEAT",
						"Died to Hitpart",
						"Died to Standard",
						"WHITELIST issue",
						"Pasting issue i see..",
						...
						"Oopsie",
						"Rawr",
						"What cheat is that? bet its not NIXUS.WIN ",
						"Ratted, it seems like ur mom is my gf",	
						"It looks like your face is on 'KILLED BY NIXUS' list.",
		...
		animations:Element("Dropdown", "animation", {options = {"floss", "default", "lil nas x", "dolphin", "monkey"}}, function(tbl) 
			...
			local valuewtr = "plazma.xyz mega pro"

			local addons = misc:Sector("addons", "Left") 

			addons:Element("Toggle", "keybinds list", nil, function(tbl) 
			...
			local watermark2 = Instance.new("ScreenGui")
						...
						TextLabel.Font = Enum.Font.Ubuntu
						TextLabel.Text = "  plazma.xyz private | Build : 1.0  |  counter-blox   |  fps 000  |  000ms  "
						TextLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
						TextLabel.TextSize = 11.000
						TextLabel.TextStrokeTransparency = 0.000
						...
						local function FHUZWEW_fake_script() -- TextLabel.LocalScript 
							...
							game:GetService('RunService').RenderStepped:Connect(function(step)
								if (tick()-lasttick)*1000 > 25 then
									lasttick = tick()				
									script.Parent.Text = "   " ..valuewtr.. "  | Build : 1.0  |  " ..GetName.Name.. "  |  " ..math.floor(1/step).. " fps | "..math.floor(game:GetService('Stats').Network.ServerStatsItem["Data Ping"]:GetValue()).. "ms   "
								end
							end)
						end
